...
static inline char max(const char a, const char b)
{
    return a > b ? a : b;
}

static inline void swap(char *const str, const size_t i, const size_t j)
{
    if (i == j)
        return;
    char tmp = str[i];
...
static inline size_t leftChild(const size_t i)
{
    return 2 * i + 1;
}

static inline size_t rightChild(const size_t i)
{
    return 2 * i + 2;
}
...
static void heapify(char *const str, const size_t str_len, const size_t root_idx)
{
    if (root_idx >= str_len)
        return;
...
static void buildHeap(char *const str, const size_t str_len)
{
...
static void heapSort(char *const str, const size_t str_len)
{
...
static void inPlaceSort(char *const str, const size_t str_len)
{
...
bool isUnique(char *const str, const size_t str_len)
{
    if (str_len < 2)
        return true;
